...
module.exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(productIds) || !productIds.every(id => mongoose.Types.ObjectId.isValid(id))) {
      ...
      return res.status(400).json({ message: 'Invalid productIds', success: false });
    }
    ...
    if (cartItem) {
      ...
      cartItem.proIds = [...new Set([...cartItem.proIds, ...productIds.map(id => new mongoose.Types.ObjectId(id))])]; // Convert to ObjectId with new
      cartItem.quantity += quantity;
      console.log('Updating existing cart item:', cartItem);
    } else {
      // If cartItem does not exist, create a new one
      cartItem = new CartItem({ userId, proIds: productIds.map(id => new mongoose.Types.ObjectId(id)), quantity }); // Convert to ObjectId with new
    ...
    return res.json({ message: 'Item(s) added to cart successfully', success: true, cartItem });
  } catch (error) {
...
module.exports.getCartItems = async (req, res) => {
  ...
  module.exports.removeCartItem = async (req, res) => {
    ...
    try {
      ...
      await cartItem.save();
      res.json({ message: 'Item removed from cart successfully', success: true, cartItem });
    } catch (error) {
